...
Phases 0-2 are done (see above); the roster has since grown past Phase 3's original 8-unit scope to 100 units, and Phase 5's art pass has started early and out of order (a real character-art batch exists for a subset of the roster, ahead of the plan's own advice in Phase 5 not to jump ahead). That's a live deviation from this doc's sequencing, not an error — but it means the next actions are no longer "start Phase 1":

1. **Playtest the actual Simulator build** on the Mac — pull latest, confirm the 100-unit roster and the new Era-tabbed deploy UI (`RoarFareContentView`) still build and run, and get a first real read on whether BU-blocking/knockback/walk-speed feel right. This is still the single most-deferred exit criterion in the whole roadmap (flagged back in Phase 1 and never actually closed).
2. **Finish the character-art pass** (blocked as of this writing on Higgsfield generation credits) — regenerate the units whose faces came out blank/mouthless-in-the-wrong-way, then the remaining new species, before doing anything else art-related.
3. Once art and a first playtest pass both exist, decide explicitly whether to keep running Phase 3 (stages, Dig Site, Fossil Record) and Phase 5 (art) in parallel like this, or pause one to let the other catch up — running them fully interleaved with no plan is how scope quietly balloons (see: 20 units becoming 100 in one sitting).
