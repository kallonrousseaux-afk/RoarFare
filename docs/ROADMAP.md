...
Phases 0-2 are done (see above); Phase 5's art pass has started early and out of order (a real character-art batch exists for a subset of the roster, ahead of the plan's own advice in Phase 5 not to jump ahead). A roster expansion to 100 units briefly happened and was then deliberately trimmed back to the 20 units with complete art (base + every evolution branch illustrated) so there's an actually-playtestable, fully-dressed roster right now — the other 80 units are fully designed and preserved in git history (commit `b18bde7`), waiting on their art before rejoining the active roster. That's a live deviation from this doc's sequencing, not an error — but it means the next actions are no longer "start Phase 1":

1. **Playtest the actual Simulator build** on the Mac — pull latest, confirm the (currently 20-unit) roster and the new Era-tabbed deploy UI (`RoarFareContentView`) still build and run, and get a first real read on whether BU-blocking/knockback/walk-speed feel right. This is still the single most-deferred exit criterion in the whole roadmap (flagged back in Phase 1 and never actually closed).
2. **Finish the character-art pass** (blocked as of this writing on Higgsfield generation credits, expected to refresh in about a week) — regenerate the units whose faces came out blank/mouthless-in-the-wrong-way, then the remaining 80 units' art, then merge that expansion back in from git history.
3. Once art and a first playtest pass both exist, decide explicitly whether to keep running Phase 3 (stages, Dig Site, Fossil Record) and Phase 5 (art) in parallel like this, or pause one to let the other catch up — running them fully interleaved with no plan is how scope quietly balloons (see: 20 units becoming 100 becoming 20 again in one sitting).
