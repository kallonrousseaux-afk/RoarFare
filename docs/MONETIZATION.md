...
- Free vs. paying player campaign-completion rate (validates §4.1 item 5's "completable without spend" requirement is actually true, not just true on paper).
...
| ~55% of new titles ship some multiplayer/PvP feature | 2026 market surveys | **Superseded — PvP was later added by explicit request.** At the time of this benchmarking pass, the recommendation was to skip PvP (Battle Cats itself has none, and PvP creates real tension with the collection-power-fantasy design elsewhere in the design doc). The user asked for it anyway afterward, so it's now a real, specified system: **`GAME_DESIGN.md` §14, "Rival Grounds."** The tension noted above didn't disappear — §14.3's stat-normalization and cosmetic-only-rewards rules exist specifically to manage it — but this row is a record of the original reasoning, not current guidance; don't take "deliberately not adopted" at face value elsewhere in this doc without checking §14 first. |

**Net effect on the two other docs (as of the original research pass):** `ART_BIBLE.md` needed no changes — none of this research touches tone/silhouette/palette. `GAME_DESIGN.md` got one addition (§13) covering Guest Dino support-borrowing and collab-ready banner framing, plus (at the time) a call-out of the PvP omission as a considered choice. That PvP call-out is now superseded per the row above — §14 is the current spec, not an omission notice.
