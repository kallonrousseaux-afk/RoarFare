...
- **The unit data model** (`Sources/RoarFareCore/UnitDefinition.swift`, `EvolutionBranch.swift`, `UnitStats.swift`) — units are data (`Resources/units.json`), not per-unit subclasses. 18 hand-authored units are bundled, covering every size class across Triassic/Jurassic/Cretaceous, the two branching-evolution examples from §5 (Deinonychus, Parasaurolophus), and three units named in §6's Pack Hunting examples that hadn't actually been built yet (Utahraptor, Styracosaurus, Pentaceratops).
