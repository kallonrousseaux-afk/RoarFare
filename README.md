...
- **The unit data model** (`Sources/RoarFareCore/UnitDefinition.swift`, `EvolutionBranch.swift`, `UnitStats.swift`) — units are data (`Resources/units.json`), not per-unit subclasses. 20 hand-authored units are bundled, covering every size class across Triassic/Jurassic/Cretaceous. Every unit has at least one evolution branch, and the roster's five legendary-rarity units (Tyrannosaurus Rex, Brachiosaurus, Spinosaurus, plus the original Deinonychus/Parasaurolophus §5 examples) each have two.
...
- Every unit's evolution branch(es) are reachable as their own deploy button, not just the base form — including the five legendary units' two branches each.
